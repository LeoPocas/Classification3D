...
from Classification3D.utils import LABEL_MAPPING, WEIGHT_PATH, OUTPUT_PATH, INCOR_RESAMPLED_PATH, TARGET_SHAPE, ZOOM, SPACING
...
class ConfusionMatrixCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        x_val_img, y_val, filenames = self.validation_data  
...
x_train_img, x_val_img, y_train, y_val, filenames_train, filenames_val = train_test_split(
    images, labels, filenames, test_size=0.15, random_state=42
)
...
class_names_sorted = [name for name, index in sorted(LABEL_MAPPING.items(), key=lambda item: item[1])]
