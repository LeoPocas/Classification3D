...
INPUT_DIR = MMs_PATH  # Diretório contendo os arquivos originais
OUTPUT_DIR = OUTPUT_PATH+'/mms_resampled/'  # Diretório para salvar os novos arquivos
folders = ['Training/Labeled', 'Training/Unlabeled', 'Validation', 'Testing']
...
for folder in folders:
    path_dir = os.path.join(INPUT_DIR, folder)
    for patient_folder in os.listdir(path_dir):
        patient_path = os.path.join(path_dir, patient_folder)
        if not os.path.isdir(patient_path):
            continue  # Ignora se não for uma pasta

        for filename in os.listdir(patient_path):
            if filename.endswith('.nii.gz') and 'gt' not in filename:  # Procura apenas arquivos 4D
